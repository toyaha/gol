...
// for postgresql
func (rec *Query) InsertDoNothingBulk() (sql.Result, error) {
	var result sql.Result
	...
	err := func() error {
		...
		if rec.Config.BulkInsertCount > 0 && rec.Value.GetValuesCount() >= rec.Config.BulkInsertCount {
			var err error
			result, err = rec.InsertDoNothing()
			if err != nil {
				return err
			}
...
// for postgresql
func (rec *Query) InsertDoNothingBulkFinish() (sql.Result, error) {
	var result sql.Result
...
// for postgresql
func (rec *Query) InsertDoUpdateBulk() (sql.Result, error) {
	var result sql.Result
...
// for postgresql
func (rec *Query) InsertDoUpdateBulkFinish() (sql.Result, error) {
	var result sql.Result
...
// for mysql
func (rec *Query) InsertIgnoreBulk() (sql.Result, error) {
	var result sql.Result
...
// for mysql
func (rec *Query) InsertIgnoreBulkFinish() (sql.Result, error) {
	var result sql.Result
...
func (rec *Query) SelectRow(dest interface{}) error {
	...
	err = rec.Client.FindRow(dest, query, valueList...)
	if err != nil {
		return err
	}
...
func (rec *Query) SelectResultRow() (*Row, error) {
	var row *Row
	...
	err := func() error {
		...
		row = rec.Client.QueryRow(query, valueList...)
